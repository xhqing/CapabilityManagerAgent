...
## 敏感信息禁止写入未被 .gitignore 忽略的文件（2026-08-02 用户立，硬性规定）

任何敏感信息——券商 / 银行账户号、API key、token、密码、私钥、数据库连接串、个人隐私标识等——**一律不得写入任何未被 `.gitignore` 忽略的文件**（即会被 git 跟踪、可能进仓库的文件），无论文件类型（代码、文档、CHANGELOG、TODO、skill 正文、README、复盘等）、无论该文件当前是否已提交。

- **为什么**：2026-08-02 事故——在 `CHANGELOG.md` / `TODO.md` 记录「老虎 paper 账户号恢复接入」过程时，把 17 位模拟账户号与 7 位实盘账户号明文写进公开仓库文档，靠 `/commit` 敏感内容扫描才发现并终止提交。敏感信息一旦进入 git 历史就是永久明文（本次两个号正是从脱敏前的旧 commit 明文恢复出来的），改文档、删文件都洗不掉。**`/commit` 的敏感内容扫描只是最后一道防线，不是第一道**——正确姿势是「写的时候就不写」，不能先写进文件、再指望扫描拦截。
- **怎么用**：
...
## 待办（todo）统一存项目根 TODO.md（2026-08-01 用户立，2026-08-02 修订）

凡「待办 / 下一步 / 代码现状（待办）」类内容——即**还没做、打算以后做**的事——只能记录到**项目根的 `TODO.md` 文件**，**禁止写到其它任何地方**（含 skill 正文 SKILL.md、references、CHANGELOG、代码注释里的 `# TODO` 等）。

- **为什么**：待办散落在规范文档 / 代码注释里，会随文档迭代过期失真（规范描述的是「现在怎么做」，夹带「以后打算做」会让读者误判当前状态），且无法统一跟踪进度。集中到 `TODO.md` 一处，既是唯一任务清单、又不污染规范与变更记录。**不再用 `todos/` 目录**——单文件就够用，少一层目录更简单，TODO.md 内部按主题分节（`##` 二级标题）即可分类，不必拆成多文件（2026-08-02 修订）。
- **怎么用**：
  - 想记一个待办 → 写到项目根的 `TODO.md`，不要写进 SKILL.md / references / CHANGELOG / 代码注释。
  ...
  - **每条待办必须附带记录时间戳（2026-08-01 用户补充，精确到分钟）**：写每一条待办时，要在条目上标注记录时间，格式是在条目标题或正文里加「`（记录：YYYY-MM-DD HH:MM）`」，**至少精确到分钟**（不能只到天）。这里的时间戳记的是**这条待办当前内容的写下 / 更新时间**，不是待办所涉及事件的发生时间——例如「平掉 7-31 误开的多仓」这条待办，误开仓发生在 7-31，但待办是在 8-01 写下的，时间戳记写下它的那一刻（精确到分）。事后改变主意、重写了这条待办的正文，时间戳要同步更新为重写那一刻。加时间戳的目的是为了能判断哪条是最新意图（见下「前后矛盾以最新为准」）。
- **前后矛盾的待办以最新时间戳为准（2026-08-01 用户补充）**：有些决定过段时间会改变主意，`TODO.md` 里可能出现对同一件事先后记了两条意图相反或不同的待办。碰到这种情况，以**时间戳最新**的那条为准——旧的那条**不删除**，只在开头打 `✅**已更新**` 标记、原条目内容不动（属下条「做完改动要闭环」的「更新」状态），并在下方另起一段新增新条目写最新意图，保留可追溯（为什么改为不删除见下条「做完改动要闭环」）。为什么以最新为准：待办清单要反映**当前最新的意图**，而不是让相互矛盾的内容同时有效、误导后续判断；时间戳精确到分钟，正是为了这种情况能分先后——同一天内先后改了两次主意，只精确到天就分不出哪条更新。
- **做完改动要闭环（查 TODO.md → 标记保留 → 记 CHANGELOG）（2026-08-02 用户修订；标记格式 2026-08-02 再修订）**：每次做完项目的任何改动后，查一下这项改动是否已在 `TODO.md` 里——若在，则把该待办**按下列两种状态之一标记（原条目内容一律保留不删）**：
  - **完成（事情真做完）**：条目开头 `[ ]` 改成 `✅**已完成**`，补完成时间戳 `（完成：YYYY-MM-DD HH:MM）`；
  - **更新（表述 / 决定被新决定取代、事还没做完）**：**原条目内容一字不动**，只在开头把 `[ ]` 换成 `✅**已更新**` + 更新时间戳 `（更新：YYYY-MM-DD HH:MM）`（标记挂在原条目前面，原标题、记录时间戳、正文都不改）；然后在下方**另起一段新增一条** `[ ]` 待办写最新决定（带新记录时间戳）。「已被取代」关系靠旧条目的 `✅**已更新**` 标记 + 相邻的新条目 + 新时间戳表达，不在原条目里塞注释。
  「已完成」「已更新」三个字**加粗**以一眼区分两种状态（2026-08-02 用户立）。**两种状态都使条目退出活跃待办——凡 `✅` 开头的条目（无论 `✅**已完成**` 还是 `✅**已更新**`）都视为已处理、看到即跳过、不再执行**；活跃待办只有 `[ ]` 开头的条目（「更新」情况下要执行的是另起一段新增的那条 `[ ]`，不是挂 `✅**已更新**` 标记的旧条目）。标记后把已做出的改动（增 / 删 / 改）记到 `CHANGELOG.md`。**不要删除已处理（完成 / 更新）的待办条目**——保留它们方便日后出问题时回溯排查（当初打算做什么、做没做、什么时候做的、后来改没改主意，都能从 TODO.md 看出来）。`TODO.md` 既留未完成、也留已处理（`✅**已完成**` / `✅**已更新**` 区分），`CHANGELOG` 记已完成变更的具体内容，两边互补不重叠。
